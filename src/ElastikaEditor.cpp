    ...
    : juce::AudioProcessorEditor(&p), processor(p)
{
  ...
  setResizable(true, true);
    ...
    //background->setTransform(juce::RectanglePlacement{128}.getTransformToFit(svgDefaultBounds, getLocalBounds().toFloat()));
    //tosvg = juce::AffineTransform::fromTargetPoints(juce::Point{0.f, 0.f}, ob.getTopLeft().toFloat(), juce::Point{60.96f, 0.f}, ob.getTopRight().toFloat(), juce::Point{60.96f, 128.5f}, ob.getBottomRight().toFloat());
    ...
    for (const auto* control : controls->getChildWithTagNameIterator("circle")) {
      ...
      if (id.endsWith("slider")) {
        ...
        kn->setTransform(juce::AffineTransform::translation(real.getX() - rounded.getX(), real.getY() - rounded.getY()));
